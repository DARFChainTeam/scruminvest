...
GroupRights: public({
                        projectID: int128,
                        floor_sum: uint256[50],
                    }[int128])


@public
def __init__(darf2eth_start: uint256, token_share_start: decimal, DARF_share_start: int128, decide_vote_start: decimal,
             thisprojecttoken: address):
...
def exchangeDARF2ETH(sum_DARF: wei_value):
    if (self.darftoken.balanceOf(msg.sender) > as_unitless_number(sum_DARF)) and (
        self.balance > sum_DARF * self.darf2eth):
...
def create_project(owner_address: address, token: address, IPFSDescribe: bytes32):
    ...
    self.Projects[ProjectID] = {projectID: ProjectID, project_owner_address: owner_address, project_token: token,
                                IPFSDescribe: IPFSDescribe}
...
@public
def setrights(ProjectID: int128, floorsum: uint256[50]):  # index:int128,floor_sum:uint256) :
    assert self.Projects[ProjectID].project_owner_address != msg.sender
    self.GroupRights[ProjectID].floor_sum = floorsum


@public
def checkrights(ProjectID: int128) -> int128:  # check access rights to project

    for grade in range(50):  #
        if self.GroupRights[ProjectID].floor_sum[grade] < self.projecttoken.balanceOf(msg.sender):
            return grade
...
def confirm_end_from_user(UserStoryID: int128):  # investors accept work
    ...
    FullStoryAmountDarf: uint256 = self.UserStory[UserStoryID].StoryAmountDarf / 100 * convert(self.DARF_share,
                                                                                               'uint256')
    # tokens 2 user
...
def userstory_fail_refund(UserStoryID: int128):
    assert self.UserStory[UserStoryID].user_address != msg.sender
    if block.timestamp > self.UserStory[UserStoryID].start_date + self.UserStory[
        UserStoryID].duration:  # deadline is broken
